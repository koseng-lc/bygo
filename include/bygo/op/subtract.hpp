...
namespace impl{
    ...
    constexpr auto subtract(in_t&& in, op_t&& op, out_t&& out, axes1_t axes1, axes2_t axes2, std::index_sequence<I...>){
        
        if constexpr(aux::is_scalar_v<util::remove_cvref_t<op_t>>){
            ::bygo::op::apply(_subtract(), std::forward<in_t>(in), std::forward<op_t>(op), std::forward<out_t>(out), axes1);
        }else{
            ::bygo::op::apply(_subtract(), std::forward<in_t>(in), std::forward<op_t>(op), std::forward<out_t>(out), axes1, axes2);
        }
    }
}
...
constexpr auto subtract(in_t&& in, op_t&& op, std::tuple<axes1_t...> axes1, std::tuple<axes2_t...> axes2){

    using out_type = util::remove_cvref_t<in_t>;

    out_type res(in);
    impl::subtract(std::forward<in_t>(in), std::forward<op_t>(op), res, axes1, axes2, Is{});

    return res;
}

template <typename in_t, typename op_t, typename ...axes_t>
constexpr auto subtract(in_t&& in, op_t&& op, std::tuple<axes_t...> axes){

    return subtract(std::forward<in_t>(in), std::forward<op_t>(op), axes, axes);
}

template <typename in_t, typename op_t>
constexpr auto subtract(in_t&& in, op_t&& op){

    using out_type = util::remove_cvref_t<in_t>;

    out_type res(in);
    apply<typename out_type::shape_type>(impl::_subtract(), std::forward<in_t>(in), std::forward<op_t>(op), res);

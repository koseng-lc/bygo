...
namespace impl{
    ...
    constexpr auto subtract(in_t&& in, op_t&& op, out_t&& out, axes1_t&& axes1, axes2_t&& axes2){
        
        ::bygo::op::apply(_subtract(), std::forward<in_t>(in), std::forward<op_t>(op), std::forward<out_t>(out)
...
constexpr auto subtract(in_t&& in, op_t&& op, axes1_t&& axes1 = whole_axes_t{}, axes2_t&& axes2 = whole_axes_t{}){

    using out_type = util::remove_cvref_t<in_t>;

    out_type res(in);
    impl::subtract(std::forward<in_t>(in), std::forward<op_t>(op), res, std::forward<axes1_t>(axes1), std::forward<axes2_t>(axes2));

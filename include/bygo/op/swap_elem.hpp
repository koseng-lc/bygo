...
namespace impl{
    ...
    constexpr auto swap_elem(in_t&& in, out_t&& out, loc1_t&& loc1, loc2_t&& loc2, std::index_sequence<I...>){
        // auto pos1_data(in(std::get<I>(loc1)...));
        // auto pos2_data(in(std::get<I>(loc2)...));

        // out(std::get<I>(loc1)...) = 1;
        out(std::get<I>(loc1)...) = in(std::get<I>(loc2)...);
        // out(std::get<I>(loc2)...) = in(std::get<I>(loc1)...);
    }
}
